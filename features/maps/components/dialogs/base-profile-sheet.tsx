...
import { LogOut, User, Mail, Phone, Clock, Edit, LucideIcon } from "lucide-react";
import { useIsMobile } from "@/hooks/use-mobile";
import { useState } from "react";
import Image from "next/image";

export interface ProfileSheetConfig {
    roleLabel: string;
    statusLabel: string;
    statusValue: string;
    ...
    const ProfileInfo = () => (
        <>
            <div className="relative shrink-0">
                <div className={`h-16 bg-linear-to-br ${config.gradientFrom} ${config.gradientTo} relative overflow-hidden`}>
                    ...
                    <div className="relative">
                        <div className="w-24 h-24 rounded-full bg-white p-1 shadow-xl">
                            {profile?.avatarUrl ? (
                                ...
                                <div className={`w-full h-full rounded-full bg-linear-to-br ${config.gradientFrom} ${config.gradientTo} flex items-center justify-center text-white text-2xl font-bold`}>
                                    {userInitials}
                                </div>
                            )}
                        </div>
                        <button className={`absolute bottom-1 right-1 w-7 h-7 ${config.primaryColor} ${config.hoverColor} rounded-full flex items-center justify-center shadow-lg transition-colors`}>
            ...
            <div className="flex-1 overflow-y-auto pt-16 px-4 pb-4">
                {isMobile ? (
                    <DrawerHeader className="text-center space-y-1 mb-4">
                        ...
                        <DrawerDescription className="text-xs text-gray-500">
                            {config.roleLabel} since {memberSince}
                        </DrawerDescription>
                    </DrawerHeader>
                ) : (
                ...
                {isLoading ? (
                    <div className="text-center py-6">
                        <div className={`inline-block w-6 h-6 border-3 ${config.primaryColor.replace('bg-', 'border-')} border-t-transparent rounded-full animate-spin`}></div>
                        <p className="text-gray-500 mt-2 text-sm">Loading profile...</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-2.5">
                            <div className={`bg-gradient-to-br from-slate-50 to-zinc-50 border-slate-100 rounded-lg p-3 border`}>
                                <div className="flex items-center gap-1.5 mb-1">
                                    <config.icon className={`w-3.5 h-3.5 ${config.primaryColor.replace('bg-', 'text-')}`} />
                                    <span className={`text-[10px] font-semibold uppercase ${config.primaryColor.replace('bg-', 'text-')}`}>{config.statusLabel}</span>
                                </div>
                                <p className="text-sm sm:text-base font-bold text-gray-900">{config.statusValue}</p>
                            </div>
                            <div className={`bg-gradient-to-br from-slate-50 to-zinc-50 border-slate-100 rounded-lg p-3 border`}>
                                <div className="flex items-center gap-1.5 mb-1">
                                    <config.activityIcon className={`w-3.5 h-3.5 ${config.primaryColor.replace('bg-', 'text-')}`} />
                                    <span className={`text-[10px] font-semibold uppercase ${config.primaryColor.replace('bg-', 'text-')}`}>{config.activityLabel}</span>
                                </div>
                                <p className="text-sm sm:text-base font-bold text-gray-900">{config.activityValue}</p>
                            </div>
                        </div>
                        ...
                        <div className={`bg-gradient-to-br from-slate-50 to-zinc-50 border-slate-100 rounded-lg p-3 border`}>
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-1.5">
                                    <Clock className={`w-4 h-4 ${config.primaryColor.replace('bg-', 'text-')}`} />
                                    <span className="text-xs font-semibold text-gray-700">{config.statsTitle}</span>
                                </div>
                            </div>
                        ...
                        <div className="space-y-2.5">
                            ...
                            <div className="space-y-2">
                                <div className="group bg-white hover:bg-gray-50 rounded-lg p-3 border border-gray-200 transition-all">
                        ...
                        <div className="space-y-2 pt-3 border-t border-gray-200">
                            <Button
                                variant="outline"
                                className="w-full flex items-center justify-center gap-2 h-9 hover:bg-gray-50 text-sm font-semibold"
                            >
                            ...
                            <Button
                                ...
                                variant="destructive"
                                className={`w-full flex items-center justify-center gap-2 h-9 ${config.primaryColor} ${config.hoverColor} text-sm font-bold`}
                            >
                                {isLoggingOut ? (
